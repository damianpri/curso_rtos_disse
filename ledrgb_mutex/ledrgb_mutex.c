...
void tarea_uart_1(void *pvParameters) {
	...
	while(1) {
		...
		xSemaphoreTake(ledrgb_mutex, portMAX_DELAY);
		ROM_GPIOPinWrite(GPIO_PORTF_BASE, LED_VERDE, LED_VERDE);
		SysCtlDelay(0.2*50000000/3);
		ROM_GPIOPinWrite(GPIO_PORTF_BASE, LED_VERDE, ~LED_VERDE);
		xSemaphoreGive(ledrgb_mutex);
...
void tarea_uart_2(void *pvParameters) {
	...
	while(1) {
		...
		xSemaphoreTake(ledrgb_mutex, portMAX_DELAY);
		ROM_GPIOPinWrite(GPIO_PORTF_BASE, LED_ROJO, LED_ROJO);
		SysCtlDelay(0.2*50000000/3);
		ROM_GPIOPinWrite(GPIO_PORTF_BASE, LED_ROJO, ~LED_ROJO);
		xSemaphoreGive(ledrgb_mutex);

		vTaskDelayUntil(&ui32WakeTime, 600 / portTICK_RATE_MS);
...
int main(void) {
	...
	ledrgb_mutex = xSemaphoreCreateMutex();
